...
def handlebooking():
    ...
    print(valid_time)
    if booking_exists:
        return jsonify({'error': 'Bạn đã đặt chuyến bay này trước đó hoặc có chuyến bay trong vòng 12 giờ.'}), 400
    if not valid_time:
        return jsonify({'error': 'Bạn đã đặt vi phạm thời gian đặt vé.'}), 400
